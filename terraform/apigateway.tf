...
resource "aws_api_gateway_method" "events_put_method" {
  authorization = "NONE"
  http_method   = "PUT"
...
resource "aws_api_gateway_integration" "events_put_integration" {
  http_method             = aws_api_gateway_method.events_put_method.http_method
  resource_id             = aws_api_gateway_resource.new_events.id
  rest_api_id             = aws_api_gateway_rest_api.api.id
  type                    = "AWS_PROXY"
...
resource "aws_api_gateway_deployment" "deploy" {
  ...
  triggers = {
    ...
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_method.events_put_method.id,
      aws_api_gateway_integration.events_put_integration.id,
